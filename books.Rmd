...
Compare users based on the books they have read
```{r}
user_rat[c(4,25,16),]
```
...
sim_users<-function(user_id,ratingmat){
  ratingmat[ratingmat==0] <- NA
rated_items <- which(!is.na((as.data.frame(ratingmat[user_id, ]))))
selected_users <- names(which(apply(!is.na(ratingmat[ ,rated_items]), 1, sum) > 2))
...
user_similarities = matrix(0, nrow = 1, ncol = length(selected_users))
  for (j in 1:length(selected_users)) {
    ...
    user_similarities[j] <- cosine_sim(ratingmat[user_id,rated_items], user_rat[selected_users[j],])
  }
...
```{r}
unread_books<-function(user_id,ratingmat){
  unread_books<-which(is.na(ratingmat[user_id,]))
  unread_books<-names(unread_books)
  return(unread_books)

}
unread_books<-unread_books(us,ratingmat)
```
